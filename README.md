...
| `eosaccess` | `arista_ceos` | `ceos:4.33.6M` | DHCP (ZTP) | Access switch |
...
1. `eosaccess` boots with ZTP enabled (via the config)
2. It sends a DHCP request on Vlan1
3. `eosdist1` relays the request (`ip helper-address 10.100.2.2`) to the CNaaS DHCP server on the management network
